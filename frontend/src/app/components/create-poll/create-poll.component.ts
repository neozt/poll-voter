      ...
      @if (createdPollId()) {
        <nz-card class="rounded-2xl shadow-lg border-0 text-center" [nzBordered]="false">
          <div class="py-8">
            ...
            <p class="text-gray-500 mb-8">Share the ID below with anyone you want to invite to vote.</p>

            <div class="bg-indigo-50 border-2 border-dashed border-indigo-300 rounded-2xl p-6 mb-8 mx-auto max-w-md">
              <p class="text-xs uppercase tracking-widest text-indigo-400 font-semibold mb-2">Poll ID</p>
              <p class="text-2xl font-mono font-bold text-indigo-700 break-all">{{ createdPollId() }}</p>
            </div>

            <div class="flex flex-col sm:flex-row gap-3 justify-center">
              <button nz-button nzType="primary" nzSize="large" class="rounded-xl bg-indigo-600 hover:bg-indigo-700 border-none font-semibold px-8" (click)="goToPoll()">
                <span nz-icon nzType="eye"></span>
                View Poll
              </button>
              <button nz-button nzType="default" nzSize="large" class="rounded-xl font-semibold px-8" (click)="copyId()">
                <span nz-icon nzType="copy"></span>
                Copy ID
              </button>
              <button nz-button nzSize="large" class="rounded-xl font-semibold px-8" (click)="resetForm()">
                Create Another
              </button>
            </div>
          </div>
      ...
      @if (!createdPollId()) {
        <nz-card class="rounded-2xl shadow-lg border-0 overflow-hidden" [nzBordered]="false">
          ...
          <form nz-form [formGroup]="pollForm" (ngSubmit)="submitForm()" nzLayout="vertical">
            ...
            <nz-form-item>
              <nz-form-label [nzSm]="24" nzRequired nzFor="title" class="font-semibold text-gray-700">Poll Question</nz-form-label>
              <nz-form-control nzErrorTip="Please provide a question for your poll!">
                <nz-input-group nzSize="large" [nzPrefix]="titleIcon">
                  <input type="text" nz-input formControlName="title" id="title" placeholder="What would you like to ask?" />
                </nz-input-group>
            ...
            <nz-form-item>
              ...
              <nz-form-control>
                <textarea nz-input formControlName="description" id="description" rows="2" placeholder="Provide additional context..."></textarea>
              </nz-form-control>
            </nz-form-item>
            ...
            <div formArrayName="options" class="space-y-3">
              @for (option of options.controls; track $index) {
                <div [formGroupName]="$index" class="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-200 group hover:border-indigo-300 transition-colors">
                  <div class="flex-shrink-0 w-8 h-8 rounded-lg bg-indigo-100 text-indigo-700 font-bold flex items-center justify-center text-sm">
                    {{ $index + 1 }}
                  </div>
                  <nz-form-item class="flex-1 mb-0">
                    <nz-form-control nzErrorTip="Required">
                      <input type="text" nz-input formControlName="title" placeholder="Option {{ $index + 1 }}" class="border-0 bg-transparent focus:bg-white rounded-lg" />
                    </nz-form-control>
                  </nz-form-item>
                  @if (options.length > 2) {
                    <button
            ...
            <div class="mt-10 pt-6 border-t border-gray-100">
              <div class="flex gap-3">
                <button nz-button nzSize="large" type="button" class="rounded-xl flex-shrink-0" routerLink="/">
                  Back
                </button>
                <button
                  nz-button
